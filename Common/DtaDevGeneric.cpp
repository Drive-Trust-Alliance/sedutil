...
uint8NOCODE(enableUserRead, char * password, char * userid)
...
uint8NOCODE(auditWrite, char * password,char * idstr)
uint8NOCODE(auditRead, char * password)
uint8NOCODE(auditErase, char * password)
uint8NOCODE(getmfgstate)
uint8NOCODE(DataStoreWrite, char * password, char * filename, uint8_t dsnum, uint32_t startpos, uint32_t len)

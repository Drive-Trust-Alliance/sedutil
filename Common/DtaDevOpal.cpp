...
uint8_t DtaDevOpal::pbaValid(char * password)
{
	...
	LOG(D) << "DtaDevOpal::pbaValid() show license level only in Linux " << dev;
...
uint8_t DtaDevOpal::activate(char * password)
{
	...
	if (disk_info.fips_fw_match && disk_info.fips_support) { // ===== FIPS initi 
		LOG(D1) << "disk_info.fips_fw_match && disk_info.fips_support are set, init FIPS"; 
	...
	// disk_info.OPAL20_numUsers
		...
		for (int i = 1; i < disk_info.OPAL20_numUsers; i++) {
			...
			LOG(D1) << "Lockingrange " << i << " ReadLock WriteLock enable";
		...
		LOG(D1) << "disk_info.fips_fw_match && disk_info.fips_support NOT set, NO init FIPS";
	}
	return lastRC;
}
...
uint8_t DtaDevOpal::DataStoreWrite(char * password, char * userid, char * filename, uint8_t dsnum, uint32_t startpos, uint32_t len)
{
	...
	if (imgsize < len)
	{
		...
		LOG(D) << "File truncated to " << len;
	}
	else
	{
		imgsize = len; // cap len to imgsize // 
		LOG(D) << "Write Length truncated " << len;
	}
	...
	delete cmd;
	delete session;
	LOG(D) << "Data Store file  " << filename << " written to " << dev;
...
uint8_t DtaDevOpal::DataStoreRead(char * password, char * userid, char * filename, uint8_t dsnum, uint32_t startpos, uint32_t len)
{
	...
	//LOG(D) << "Read Data store from " << dev << " to " << filename;
	while ((filepos + startpos) < disk_info.DataStore_maxTableSize && (filepos < len)) {
	...
	LOG(D) << "Read Data Store from "<< dev << " to " << filename << " OK";
...
uint8_t DtaDevOpal::MBRRead(char * password, char * filename, uint32_t startpos, uint32_t len)
{
	...
	LOG(D) << "Read MBR from " << dev << " to " << filename << " " << dev;
...
uint8_t DtaDevOpal::getMBRsize(char * password)
{
	...
	if (skip_activate) {
		...
		LOG(D) << " start non-authenticated session with ADMINSP OK for printdefaultpassword";
		if ((lastRC = session->start(OPAL_UID::OPAL_ADMINSP_UID)) != 0) {
			delete session;
			LOG(D) << " Failed : start non-authenticated session with ADMINSP OK for printdefaultpassword";
			return lastRC;
		}
		LOG(D) << " OK : start non-authenticated session with ADMINSP OK for printdefaultpassword";
...
uint8_t DtaDevOpal::loadPBA(char * password, char * filename) {
	...
	if (skip_activate)
		return (loadPBA_O(password, filename));
	else
		return(loadPBA_M(password, filename));
}
// from original

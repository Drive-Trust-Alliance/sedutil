...
static uint8_t acquireDiscovery0Response(DtaDevLinuxDrive * d, uint8_t * d0Response)
{
                              ...
                              DtaDevOS * & dev, bool genericIfNotTPer)
{
  ...
  uint8_t d0Response[MIN_BUFFER_LENGTH];
  memset(d0Response, 0, MIN_BUFFER_LENGTH);

  uint8_t lastRC = acquireDiscovery0Response(drive, d0Response);
  if ((lastRC ) != 0) {
    delete drive;
    dev = NULL;

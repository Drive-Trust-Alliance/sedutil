                              ...
                              bool genericIfNotTPer)
{
  ...
  DtaDevOS * d = new DtaDevGeneric(devref);

 if (!strncmp(devref, "/dev/nvme", 9))
    {
      //		DtaDevLinuxNvme *NvmeDrive = new DtaDevLinuxNvme();
      d->drive = new DtaDevLinuxNvme();
    }
  else if (!strncmp(devref, "/dev/s", 6))
    {
      //		DtaDevLinuxSata *SataDrive = new DtaDevLinuxSata();
      d->drive = new DtaDevLinuxSata();
    }
  else
    {
      dev = NULL;
 ...
 if (!(d->drive->init(devref)))
  ...
  d->drive->identify(d->disk_info);


#define TestClass(test,cls)                     \
  if (d->test())                                \
    {                                           \
      delete d;                                 \
      dev = new cls(devref);                    \

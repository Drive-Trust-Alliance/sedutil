        ...
        if not noPW and not wrongPW and not e.is_set():
            if ui.DEV_OS == 'Windows':
                ...
                if ui.process != None:
                    ...
                    gobject.idle_add(rt_setupUSB_cleanup, ui, index, status1, noPW, s, e, preserved_files, present, rescan_needed)
                else:
                    gobject.idle_add(rt_setupUSB_cleanup, ui, index, -1, noPW, s, e, preserved_files, present, rescan_needed)
            ...
            gobject.idle_add(rt_setupUSB_cleanup, index, ui, -1, noPW, s, e, preserved_files, present, rescan_needed)
        runop.postlock(index)
    else:
        gobject.idle_add(rt_setupUSB_cleanup, index, ui, -1, noPW, s, e, preserved_files, present, rescan_needed)

def rt_setupUSB_cleanup(ui, index, status, no_pw, s, e, preserved_files, present, rescan_needed):
    pass_usb = ''
...
def rt_setupUser(ui, index, e):
    ...
    if present:
        if ui.VERSION == 3 and ui.check_pass_rd.get_active():
            ...
            (password_a, pass_usb) = runprocess.passReadUSB("Admin", ui.dev_select.get_active_text(), ui.dev_vendor.get_text(), ui.dev_sn.get_text())
        ...
        if e.is_set():
            notify_TO = True
            gobject.idle_add(rt_setupUser_cleanup, ui, index, -1, notify_TO, noPW, noUSB, password_a, password_u, present, rescan_needed, save_status)
            ...
            if e.is_set():
                ...
                if ui.datastore_list[index] == 'Supported':
                    ...
                    runop.postlock(index)
                gobject.idle_add(rt_setupUser_cleanup, ui, index, s3, notify_TO, noPW, noUSB, password_a, password_u, present, rescan_needed, save_status)
        else:
            gobject.idle_add(rt_setupUser_cleanup, ui, index, -1, notify_TO, noPW, noUSB, password_a, password_u, present, rescan_needed, save_status)
    else:
        gobject.idle_add(rt_setupUser_cleanup, ui, index, -1, notify_TO, noPW, noUSB, password_a, password_u, present, rescan_needed, save_status)

def rt_setupUser_cleanup(ui, index, status, op_to, no_pw, no_usb, password_a, password_u, present, rescan_needed, save_status):
    pass_usb = ''
...
def rt_removeUser(ui, index, e):
    ...
    if present:
        ...
        #elif m_no:
        #    rc = None
    gobject.idle_add(rt_removeUser_cleanup, ui, index, rc, notify_TO, no_pw, password_a, pass_usb, present, rescan_needed, save_status)

def rt_removeUser_cleanup(ui, index, status, op_to, no_pw, password_a, pass_usb, present, rescan_needed, save_status):

    ...
    def t1_run(e, dev, i, thread_list, result_list, status_list, count, password, hash_au, autounlock, reboot):
        ...
        if not e.is_set() and not noPW:
            if not autounlock:
                ...
                if ui.VERSION % 2 == 1 and user_found and not admin_found:
                    status1 = os.system(ui.prefix + 'sedutil-cli -n -t -u --setLockingRange 0 ' + ui.LKATTR + ' "' + password + '" ' + dev)
                    if status1 != 0:
                        status_final = status1
                    elif e.is_set():
                ...
                if ui.DEV_OS == 'Windows':
                    with open(os.devnull, 'w') as pipe:
                        if status_final != 0 or noPW:
                            ...
                            if ui.VERSION % 2 == 1 and ((not autounlock and auth == 1) or (autounlock and auto_auth == 1)):
...
def rt_revertPSID(ui, psid):
    ...
    if present:
        ...
        t_aw.start()
        time.sleep(0.5)
    gobject.idle_add(rt_revertPSID_cleanup, ui, status, present, rescan_needed)

def rt_revertPSID_cleanup(ui, status, present, rescan_needed):
    index = ui.dev_select.get_active()
    ...
    if not failed:
        ...
        if not failed:
            ...
            if not failed:
                ...
                #print add_str
        runop.postlock(index)
    gobject.idle_add(rt_queryAuth_cleanup, self, index, failed, parent, add_str, rc, no_pass, no_usb)
    
def rt_queryAuth_cleanup(self, index, failed, parent, add_str, rc, no_pass, no_usb):
...
def rt_openLog(parent, index):
    ...
    if present:
        ...
        if (parent.VERSION == 3 or (parent.VERSION == 1 and parent.PBA_VERSION != 1)) and parent.check_pass_rd.get_active():
            (password, pass_usb) = runprocess.passReadUSB(parent.auth_menu.get_active_text(), parent.dev_select.get_active_text(), parent.dev_vendor.get_text(), parent.dev_sn.get_text())
            rd_usb = True
            if password == None or password == 'x':
                no_pw = True
            ...
            if parent.DEV_OS == 'Windows':
                ...
                if (parent.VERSION == 3 or (parent.VERSION == 1 and parent.PBA_VERSION != 1)) and parent.datastore_list[index] == 'Supported':
                    ...
                    if statusAW == 0:
                        ...
                        os.system(parent.prefix + 'sedutil-cli -n -t -u --auditwrite 09' + timeStr + ' "' + hash_pwd + '" User' + parent.user_list[index] + ' ' + parent.devname)
    runop.postlock(index)
    gobject.idle_add(rt_openLog_cleanup, parent, index, statusAW, password, txt, no_pw, no_USB, not_detected, not_setup, not_active, auth_level, drive, rescan_needed, present, save_status)

def rt_openLog_cleanup(parent, index, statusAW, password, txt, no_pw, no_USB, not_detected, not_setup, not_active, auth_level, drive, rescan_needed, present, save_status):
    if not present:

...
def hash_pbkdf2(plaintext, salt, iterations = 75000, bytes = 32):
    ...
    pwhash = PyExtHash.hashpwd(plaintext,salt, iterations, bytes)
    return pwhash

def hash_pass(plaintext, dev_idx, ui):#salt, msid):
    msid = ui.msid_list[dev_idx]
    salt = ui.salt_list[dev_idx]

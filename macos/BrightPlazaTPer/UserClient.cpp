...
IOReturn UserClientClass::clientClose(void)
{
    ...
    UC_IOLOG_DEBUG_METHOD("Call terminate");
    bool success = terminate();
    if (!success) {
        IOLOG_DEBUG_METHOD("terminate() failed");
    }
    
    UC_IOLOG_DEBUG_METHOD("DON'T call super::clientClose, which just returns kIOReturnUnsupported.");
...
{
    UC_IOLOG_DEBUG_METHOD("*** before super(" REVEALFMT ")", REVEAL(provider));
    super::stop(provider);
